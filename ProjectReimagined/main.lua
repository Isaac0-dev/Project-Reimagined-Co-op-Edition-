...
function new_level_title_wmotr_1()
    local np = gNetworkPlayers[0]
if np.currLevelNum == LEVEL_WMOTR and
np.currAreaIndex == 1 then

    djui_popup_create("You've entered Big Boo's Secret Domain...beware!",3)
  end 
end
hook_event(HOOK_ON_LEVEL_INIT, new_level_title_wmotr_1)

function new_level_title_wmotr_2()
    local np = gNetworkPlayers[0]
if np.currLevelNum == LEVEL_WMOTR and
np.currAreaIndex == 2 then

    djui_popup_create("You've entered Platform Peril! Watch your step!",3)
  end 
end
hook_event(HOOK_ON_LEVEL_INIT, new_level_title_wmotr_2)

function new_level_title_pss_1()
    local np = gNetworkPlayers[0]
if np.currLevelNum == LEVEL_PSS and
np.currAreaIndex == 1 then

    djui_popup_create("You've entered the Battle Fort!",3)
  end 
end
hook_event(HOOK_ON_LEVEL_INIT, new_level_title_pss_1)

function new_level_title_pss_2()
    local np = gNetworkPlayers[0]
if np.currLevelNum == LEVEL_PSS and
np.currAreaIndex == 2 then
...
hook_event(HOOK_ON_LEVEL_INIT, new_level_title_pss_2)

function new_level_title_sa_1()
    local np = gNetworkPlayers[0]
if np.currLevelNum == LEVEL_SA and
np.currAreaIndex == 1 then

    djui_popup_create("You've entered Goomboss' Lair...its King's Whomps' Lair now!",3)
  end 
end
hook_event(HOOK_ON_LEVEL_INIT, new_level_title_sa_1)

function new_level_title_hmc_3()
  local np = gNetworkPlayers[0]
if np.currLevelNum == LEVEL_HMC and
np.currAreaIndex == 3 then

  djui_popup_create("You found the back entrance to the Crystal Caverns!",3)
end 
end
hook_event(HOOK_ON_LEVEL_INIT, new_level_title_hmc_3)